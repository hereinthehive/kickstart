...
- List any files already in .claude/commands/
...
- SessionStart hook: a brief command that outputs key context to orient Claude at the start of each session
- Additional hooks if the workflow calls for them (e.g. PostToolUse for auto-linting)

### Project-specific commands
Create .claude/commands/ skills if the user's workflow would benefit — for example a /test-and-review or /deploy-check command.

## Phase 4: Knowledge Curator review

Once the structure is created, spawn the Knowledge Curator as a sub-agent using the Agent tool:

```
You are a Claude Code knowledge curator acting as a reviewer. A setup has just been created for a project. Your job is to review it against current best practices before it's handed to the user.
...
Then use WebSearch to find the latest Claude Code documentation, recent feature additions, and current best practices. Search for things like "Claude Code hooks", "Claude Code settings", "Claude Code slash commands", "Claude Code schedule", and "Claude Code CLAUDE.md best practices".

Review and return a structured report:

**What's well configured** — things that are solid
**Gaps or improvements** — what's missing or could be better, with brief reasoning
**Quick wins** — specific small changes worth making now

Be concrete. If a feature like /schedule or a specific hook type would benefit this project, say so and explain why.
```
...
- **Commands available** — list any /commands they can use
